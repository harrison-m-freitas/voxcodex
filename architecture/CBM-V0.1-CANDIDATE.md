# Canonical Book Model v0.1 Candidate — Contrato Normativo Congelado

**Status:** `FROZEN — candidate para blind holdout conformance`  
**Schema version:** `0.1-candidate`  
**Freeze date:** `2026-09-11`  
**Predecessor:** `CBM v0.1-alpha candidate` (cross-genre validated)  
...
**Freeze:** realizado em 2026-09-11  
**Holdouts cegos:** não consultados durante o freeze; liberados apenas para o próximo gate de blind conformance
...
| `fidelity_manifest_ref` | 0..1 | opcional no candidate |
...
Eventos semânticos permanecem annotations no candidate, em vez de virar entidade/event type de primeira classe.
...
- nenhuma AST matemática universal é escolhida no `v0.1 candidate`.
...
Para `lifecycle_state = frozen`, também são obrigatórios `source_mapping_manifest_ref`, `provenance_manifest_ref` e `content_digest`. `fidelity_manifest_ref` permanece opcional no candidate.
...
# 25. Conformidade v0.1 Candidate congelado

Uma implementação só pode alegar conformidade conceitual com o **`CBM v0.1 candidate` congelado em 2026-09-11** se suportar pelo menos:
...
# 26. Decisões de freeze e itens deliberadamente adiados

O freeze review resolveu os pontos normativos que poderiam criar ambiguidade no candidate:
...
Permanecem deliberadamente adiados, **sem bloquear o blind holdout**:
...
Qualquer necessidade de alteração normativa descoberta pelos holdouts **não modifica este candidate congelado**: deve produzir um novo candidate/versionamento explícito e reiniciar o gate de conformidade correspondente.
...
O candidate está congelado para o teste cego. O próximo gate é:

```text
abrir F-RND p.33 + W-RND p.558
        ↓
blind conformance
        ↓
PASS / PASS_WITH_EXTENSION / MODEL_GAP / MODEL_FAILURE
```

Os holdouts não foram consultados durante o freeze review.
