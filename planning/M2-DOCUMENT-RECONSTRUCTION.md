...
- milestone: **M2**
- nome: **Document Reconstruction & CBM Materialization**
- estado atual: **M2 Design v0.1 corrigido após review consolidado e congelado**
- M2.1 — Boundary & Success Contract: **APROVADO / FROZEN**
- M2.2 — Source Artifact & Evidence Model: **APROVADO / FROZEN**
- M2.3 — Reconstruction Pipeline: **APROVADO / FROZEN**
- M2.4 — CBM Materialization & Validation: **APROVADO / FROZEN**
- M2.5 — Reprocessing, Confidence & Provenance: **APROVADO / FROZEN**
- M2.6 — Compatibility Corpus & Exit Criteria: **APROVADO / FROZEN**
- próximo gate: **revisão humana do contrato congelado e plano de implementação**
- baseline normativa: `architecture/CBM-V0.1.md`
- stack tecnológica: **não selecionada**

> O freeze final do `M2 Design v0.1` incorpora as emendas corretivas do review consolidado sem alterar o `CBM v0.1`. O snapshot histórico que congelava M2.1–M2.3 permanece preservado. Stack, serialização física e implementação continuam não selecionadas.
...
As seis seções arquiteturais estão aprovadas e o review consolidado foi resolvido. O contrato normativo final está congelado em `architecture/M2-DESIGN-V0.1.md`. Próximo gate: revisão humana do contrato congelado e, após aprovação, plano de implementação.

O snapshot histórico M2.1–M2.3 permanece preservado por `M2-DESIGN-FREEZE-MANIFEST.json`; nenhuma decisão tecnológica foi selecionada.


---

# Emendas corretivas do review consolidado — incorporadas no freeze final

As seguintes decisões resolvem `B1–B5` e `Q1–Q8` de `planning/M2-CONSOLIDATED-DESIGN-REVIEW.md` sem alterar o `CBM v0.1`:

1. `Derivation` é autoridade exclusiva das arestas causais de provenance; `ProcessingActivity` permanece metadata/unidade de execução.
2. `ReconstructionUnit.parent_ref + order_key` são autoridade hierárquica; `child_refs` é projeção derivável (`R13`).
3. `CanonicalTargetContext` operacional fornece/provisiona de forma auditável `Work`/`Edition`/`CanonicalDocument` antes de R1.
4. `validation_report_ref` de revision frozen identifica o freeze-authorizing report e é imutável; revalidation posterior gera reports suplementares externos.
5. Blind protocol congela assertions/holdout selection antes do tuning e candidate identity/config/claims antes do reveal (`C15–C16`).
6. `stale` é contextual, não mutação.
7. Contratos operacionais M2 não são primitivas CBM.
8. Reconstruction validation é operacional e distinta do CBM `ValidationReport`.
9. Significância de evidence é policy-controlled/versionada.
10. Compatibility gates são avaliados por support tier.
11. `MODEL_GAP` exige rebaseline explícito se a baseline CBM mudar.
12. Evidence/provenance referenciada por frozen revisions permanece endereçável conforme retention/audit policy.
13. O contrato normativo congelado final é `architecture/M2-DESIGN-V0.1.md`; manifests históricos anteriores permanecem auditáveis.
