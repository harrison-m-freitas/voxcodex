...
class Annotation(FrozenModel):
    ...
    def inferred_annotation_has_evidence(self) -> "Annotation":
        if "." not in self.annotation_type:
            raise ValueError("annotation_type must be namespaced")
...
class Relation(FrozenModel):
    ...
    @model_validator(mode="after")
    def inferred_relation_has_evidence(self) -> "Relation":
        if "." not in self.predicate:
            raise ValueError("predicate must be namespaced")
        if self.epistemic_status == "inferred" and not self.evidence_refs:
